...
class PerModuleGradDumper(TrainerCallback):
    ...
    def on_pre_optimizer_step(self, args, state, control, **kwargs):
        ...
        layers = None
        for cand in ["model.model.layers",
                     "model.layers",
                     "transformer.h",
                     "gpt_neox.layers",
                     ...
                     "encoder.layer",  # bare RobertaModel
        ]:
            try:
        ...
        def _get_first(obj, dotted_paths):
            """
            Return the first attribute/module found by trying a list of dotted paths.
            Example: _get_first(blk, ["attention.self.query", "self_attn.q_proj"])
            """
       ...
       # Iterate layers and collect standard projections (RoBERTa/BERT + GPT-style)
        for lid, blk in enumerate(layers):
            # --- Attention projections ---
            # RoBERTa/BERT names:
            #   blk.attention.self.query / key / value (Linear)
            #   blk.attention.output.dense (Linear)
            # GPT/LLaMA-style fallbacks:
            #   blk.self_attn.q_proj / k_proj / v_proj / o_proj
            #   or packed qkv variants
            q = _get_first(blk, ["attention.self.query", "self_attn.q_proj"])
            k = _get_first(blk, ["attention.self.key",   "self_attn.k_proj"])
            v = _get_first(blk, ["attention.self.value", "self_attn.v_proj"])
            o = _get_first(blk, ["attention.output.dense", "self_attn.o_proj"])

            # Also try fused QKV variants in decoder-style models
            qkv = _get_first(blk, [
                "self_attn.qkv", "self_attn.qkv_proj", "attention.qkv", "attention.qkv_proj"
            ])

            found_any_attn = False
            for nm, sub in [
                ("q_proj", q), ("k_proj", k), ("v_proj", v), ("o_proj", o),
                ("qkv", qkv), ("qkv_proj", qkv),
            ]:
                if sub is None:
                    continue
                found_any_attn = True
            ...
            if not found_any_attn:
                print("attn is None or unsupported layout in this block")

            # --- MLP projections ---
            # RoBERTa/BERT names:
            #   blk.intermediate.dense  (≈ up)
            #   blk.output.dense        (≈ down)
            # GPT/SwiGLU fallbacks:
            #   blk.mlp.gate_proj / up_proj / down_proj
            gate = _get_first(blk, ["mlp.gate_proj"])  # may be absent in RoBERTa
            up   = _get_first(blk, ["mlp.up_proj", "intermediate.dense"])
            down = _get_first(blk, ["mlp.down_proj", "output.dense", "ffn.output"])  # extra fallback

            found_any_mlp = False
            for nm, sub in [("gate_proj", gate), ("up_proj", up), ("down_proj", down)]:
                if sub is None:
                    continue
                found_any_mlp = True
                if hasattr(sub, "weight"):
            ...
            if not found_any_mlp:
                print("mlp is None or unsupported layout in this block")
