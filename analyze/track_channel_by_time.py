...
def load_grad_channels_for_step(grad_base_dir: str, step: int, 
    ...
    # DEBUG: Print layer-wise channel counts
    print(f"\n[DEBUG] Layer-wise channel counts for step {step}:")
    for lid in sorted(mlp_energy_per_layer.keys()):
        d = mlp_energy_per_layer[lid]
        inc_size = d.get("incoming", np.array([])).size
        out_size = d.get("outgoing", np.array([])).size
        print(f"  Layer {lid}: incoming={inc_size}, outgoing={out_size}")
    print(f"[DEBUG] Total layers with data: {len(mlp_energy_per_layer)}")
    print(f"[DEBUG] Total channels collected: {sum(arr.size for arr in mlp_all)}\n")
