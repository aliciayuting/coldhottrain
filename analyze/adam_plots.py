...
OUTPUT_BASE = os.path.join(SCRIPT_DIR, "adam_norms_byhead")
...
def get_index_to_param_map(checkpoint_path):
    ...
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if name in decay:  # weight decay group first
            if "query" in name or "value" in name or "classifier" in name or FULL_FT:
                ordered.append((name, p))
    for name, p in model.named_parameters():
        if not p.requires_grad:
            continue
        if name not in decay:  # then the no-decay group
            if "query" in name or "value" in name or "classifier" in name or FULL_FT:
                ordered.append((name, p))

    return dict(enumerate(ordered)), model.config


def compute_headwise_column_norms(tensor: torch.Tensor, head_info):
    if tensor.ndim != 2 or head_info is None:
        return None
    ...
    if rows != expected_rows:
        return None
    cols = tensor.shape[1]
    view = tensor.reshape(num_heads, head_dim, cols)
    # Norm across the per-head rows while keeping heads/columns separate
    headwise = torch.linalg.norm(view, dim=1)
    # Reorder so that each column contributes num_heads consecutive lines
    return headwise.permute(1, 0).reshape(-1)
...
def compute_row_column_norms(tensor: torch.Tensor, head_info=None):
    ...
    if tensor.ndim >= 2:
        if head_info:
            col_norms = compute_headwise_column_norms(tensor, head_info)
...
def main():
    ...
    for iter_num, dir_name in checkpoint_dirs:
        if iter_num % EVERY_NTH != 0:
            continue
        checkpoint_path = os.path.join(PATH, dir_name)
        print(f"Analyzing {dir_name} (iteration {iter_num})")
        if index_to_param is None:
            index_to_param, model_config = get_index_to_param_map(checkpoint_path)

        optimizer_path = os.path.join(checkpoint_path, "optimizer.pt")
        opt_state = torch.load(optimizer_path, map_location="cpu")
        state_dict = opt_state.get("state", {})

        for param_id, s in state_dict.items():
            if param_id not in index_to_param:
                continue
            param_name, param_template = index_to_param[param_id]
            exp_avg = s.get("exp_avg")
            if exp_avg is None:
                continue
            if param_template.shape != exp_avg.shape:
                raise ValueError(
                    f"Shape mismatch for param_id {param_id} ({param_name}): expected {param_template.shape}, got {exp_avg.shape}"
                )

            entry = stats[param_name]
            if not entry["head_info_initialized"]:
                entry["head_info"] = build_head_split_info(param_name, tuple(param_template.shape), model_config)
                entry["head_info_initialized"] = True
            head_info = entry["head_info"]

            row_norms, col_norms = compute_row_column_norms(exp_avg, head_info)
